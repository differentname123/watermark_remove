...
def gen_cut_suggestion(video_path):
    ...
    try:
        ...
        prompt = """# 角色
                    ...
                    4.  **【优化】效果是唯一标准 (Impact is Everything)**：所有决策的唯一目标是让最终成片在**观众吸引力、叙事流畅性、信息价值和传播潜力**上获得**显著提升**。 **“显著提升”不仅指信息传递效率的提高，更包括观看体验的优化、情绪铺垫的完整性和叙事节奏的舒适度。一个节奏良好、详略得当的30秒视频，其综合传播效果可能远胜于一个信息密集但观感仓促的10秒视频。**
                    5.  **内容安全优先**：如果该场景中有不相关的内容（如广告、推广，甚至是关于我的声明或者介绍）不管是不是我推荐的，那么这个场景都应该被删除。立即识别并删除任何包含我的个人身份信息（如Logo、个人头像、姓名、联系方式等）的场景。这是强制性要求。
                    6.  **务实剪辑，反对炫技 (Practicality over Complexity)**：要尽量不要产生太多的剪切点，因为会导致难度大大提升。我非常反感为了改而改的微小调整。如果原始顺序已经足够好，或者任何改动带来的提升都微不足道，请明确指出应保留原顺序，并解释原因。减少不必要的剪辑工作量是一个重要的考量点。
                    7.  **【优化】完整体验原则 (Complete Experience Principle)**：视频不仅要传递信息，更要提供一段**完整的观看体验**。剪辑方案必须保证最终成片具有**合理的时长和叙事结构**。一个只有几秒的视频可能信息密度极高，但作为一条‘新闻资讯’或‘故事讲解’，它会因过短而显得不完整、缺乏说服力。**你的首要任务是产出一个“成立的”视频作品。对于新闻类视频，一个“标题+详情”的结构是基础，删除提供证据或细节的“详情”部分，是严重的策略错误。**
                    ...
                    2.  **【核心优化】逐一评估 (Scene-by-Scene Evaluation)**：
                        *   结合你对整体的理解，独立评估每个原始场景。评估维度包括：
                            *   **信息密度**：是否传递了关键信息？
                            *   **视觉冲击力**：画面是否吸引人？
                            *   **情绪价值**：能否引发观众的情绪（好奇、共鸣、兴奋、爽感等）？
                            *   **叙事功能**：在故事中扮演什么角色（开端、发展、高潮、结尾、铺垫、转折）？
                            *   **冗余性分析**：**【重点】** 评估内容是**“破坏性冗余”**（例如，完全相同的口播或画面反复出现，毫无新意，只会拖慢节奏），还是**“建设性补充”**（例如，场景1提出观点/宣布结果，场景2提供论据/展示细节）。**对于“建设性补充”的场景，即使内容与核心信息有重叠，也必须予以保留，因为它对于构建完整的叙事、增强信息的可信度至关重要。**
                    
                    3.  **制定剪辑策略 (Formulate the Editing Strategy)**：
                        *   严格遵循上述**【核心原则】**，结合你的评估，构建最终剪辑方案。
                        *   对于每一个决策（保留、重排、删除），在`reasoning`中清晰阐述你的思考过程。
                        *   **【新增】** 当你根据“建设性补充”原则决定**保留**一个场景时，你的`reasoning`必须明确解释为什么这个场景虽然看似重复，但对视频的**完整性、可信度或节奏感**是不可或缺的。例如：“保留。虽然核心信息‘Wei加入iG’已在场景1宣布，但此场景展示了官方公告原文，是新闻真实性的关键支柱。删除它会使视频变成一条缺乏根据的‘短讯’，而不是一条可信的‘新闻’。它的存在是构建完整叙事和观众信任的必要环节。”
                    ...
                    *   **内容结构**：JSON对象必须包含以下三个顶级键：`overall_strategy`, `final_cut_sequence`, `deleted_scenes`。
                    ...
                    ---
                    ### **JSON输出格式定义与示例 (已恢复为简单模式)**
                    
                    ```json
                    {
                      "overall_strategy": "（这里是你基于【核心原则】和【整体诊断】得出的顶层策略。例如：原始视频采用‘宣布核心新闻-展示官方细节’的经典叙事结构，逻辑清晰。场景2虽然信息密度低于开场，但作为新闻真实性的支撑是必要的。因此，最佳策略是保留原始结构，不进行删改，以保证视频的完整性和可信度。）",
                      "final_cut_sequence": [
                        {
                          "scene_id": "场景1",
                          "scene_desc": "（场景的简短描述）",
                          "reasoning": "（你的决策理由。例如：作为视频的开端，高效宣布核心新闻，抓住观众注意力，是完美的‘标题’。）"
                        },
                        {
                          "scene_id": "场景2",
                          "scene_desc": "（场景的简短描述）",
                          "reasoning": "（例如：保留。此场景属于‘建设性补充’。它通过展示官方公告，为场景1的新闻提供了关键的‘证据’和可信度。虽然时长较长，但删除后会导致视频信息来源不明，严重损害其作为新闻的价值。因此必须保留，以构成完整的叙事链条。）"
                        }
                      ],
                      "deleted_scenes": [
                        {
                          "scene_id": "场景X",
                          "scene_desc": "（场景的简短描述）",
                          "reasoning": "（你的决策理由。例如：此场景属于‘破坏性冗余’，内容与场景Y完全重复，且无新的视觉或信息增量，删除后可使节奏更紧凑。）"
                        }
                      ]
                    }
                    ```
