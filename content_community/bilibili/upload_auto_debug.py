...
from common_utils.video_utils import add_image_to_video_end, get_video_duration_seconds, create_enhanced_cover, \
    merge_videos_ffmpeg, apply_all_subtle_tweaks, process_video_with_template
...
# ---------- 文件路径常量 ----------
UPLOAD_LOG_FILE = '../../LLM/TikTokDownloader/metadata_cache_with_uploads_fix.json'  # 上传日志
persistent_tasks_file = "../../LLM/TikTokDownloader/persistent_tasks_fix.json"
UPLOAD_LOG_FILE_ORIGIN = '../../LLM/TikTokDownloader/metadata_cache_with_uploads.json'  # 上传日志
...
def add_template(video_path, output_video_path, user_name):
    ...
    """
    try:
        template_data = read_json('template.json')
        user_template_info = template_data.get(user_name, {})
        if not user_template_info:
            print(f"⚠️ 用户 {user_name} 的模板信息不存在，使用默认模板。")
            return False
        template_image = f"{user_name}_transparent.png"
        left_up_point = user_template_info.get('left_up_point', (0, 0))
        box_info = user_template_info.get('box_info', (0, 0))
...
def auto_upload():
    ...
    upload_log_origin: dict = load_json(UPLOAD_LOG_FILE_ORIGIN, default={})
    ...
    # 2. 遍历权威元数据
    for key, value in upload_log.items():
        if key in persistent_tasks:
            print(f"🔍 跳过 {key}：已在持久化任务列表中，可能是之前的上传失败或中断。")
            continue
        ...
        if os.path.exists(temp_video_path):
            ...
            print(f"✅ 添加模板成功，保存为 {temp_video_path}")
        ...
        if new_uploads_made:
            try:
                ...
                persistent_tasks.add(key)  # 添加当前任务到持久化任务列表
                save_json(persistent_tasks_file, list(persistent_tasks))
                upload_log_origin.update(upload_log)
                save_json(UPLOAD_LOG_FILE_ORIGIN, upload_log_origin)
